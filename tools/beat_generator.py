...
from tools.genre_resolver import resolve_genre, genre_to_music_theory
...
async def generate_beat(
    ...
    # ── Resolve genre — known DB first, Claude API as fallback ───────────────
    genre_is_known = genre_lower in DRUM_PATTERNS and genre_lower in CHORD_PROGRESSIONS
    resolved_data: dict | None = None

    if not genre_is_known:
        if ctx:
            await ctx.info(f"Genre '{genre}' not in local DB — asking Claude API...")
        resolved_data = resolve_genre(genre_lower)
        if resolved_data:
            mt = genre_to_music_theory(resolved_data)
            # Inject into music_theory dicts so downstream functions find it
            DRUM_PATTERNS[genre_lower] = mt["drum_pattern"]
            CHORD_PROGRESSIONS[genre_lower] = mt["chord_progression"]
            GENRE_BPM[genre_lower] = mt["bpm_range"]
            MELODY_RHYTHMS[genre_lower] = mt["melody_rhythms"]
            if ctx:
                await ctx.info(f"  Resolved via Claude API — cached for future use")
        else:
            if ctx:
                await ctx.info(f"  Could not resolve '{genre}' — falling back to hip hop template")
    ...
    # Genre instrument knowledge — prefer Claude-resolved data if available
    if resolved_data and resolved_data.get("instruments"):
        genre_instruments = resolved_data["instruments"]
    else:
        genre_instruments = get_genre_instruments(genre_lower)
    ...
    result = {
        ...
        "instruments": genre_instruments,
    }

    if resolved_data:
        result["genre_source"] = "claude_api"
        result["genre_note"] = f"'{genre}' was not in the local database — pattern generated by Claude and cached for future use."
